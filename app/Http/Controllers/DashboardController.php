    ...
    public function index()
{
    ...
    // 6. Hitung jumlah hadir & tidak hadir
    $jumlahHadir = $data->whereIn('status', ['Hadir', 'Hadir (Terlambat)'])->count();

    // Tidak hadir = total user - (yang hadir + cuti + lembur)
    $jumlahCuti = $data->where('status', 'Cuti (Disetujui)')->count();
    $jumlahLembur = $data->where('status', 'Lembur (Disetujui)')->count();
    $jumlahTidakHadir = $allUser - ($jumlahHadir + $jumlahCuti + $jumlahLembur);

    // 7. Kirim data ke view
    return view('dashboard.hr', compact('data', 'allUser', 'jumlahHadir', 'jumlahTidakHadir'));
}
    ...
    $lemburDisetujui = Lembur::where('user_id', $user->id)
            ->where('status_pengajuan', 'disetujui')
            ->count();
        return view('dashboard.direktur', compact('user', 'cutiDisetujui', 'lemburDisetujui', 'absensiHariIni'));
    }
    ...
    public function atasan()
    {
     ...
     $cutiDisetujui = Cuti::where('user_id', $user->id)
        ...
        return view('dashboard.atasan', compact('user', 'cutiDisetujui', 'lemburDisetujui', 'absensiHariIni'));
    }
